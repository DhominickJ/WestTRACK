      ...
      {imgPos === "left" && (
        <div className={`flex items-center justify-center bg-${bgColor}`}>
          <Image
            ...
            className="left"
          />
          <div className="pl-10 flex flex-col justify-center items-left w-1/2 text-white">
            <h3 className="font-bold text-4xl pb-5">{header}</h3>
            <p className="w-1/2 text-lg">{content}</p>
          </div>
        </div>
      )}
      {imgPos === "right" && (
